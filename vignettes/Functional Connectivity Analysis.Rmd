...
1. [fMRI Preprocessing](#fmri-preprocessing)
    + [Motion Correction](#motion-correction)

2. [Connectivity Preprocessing](#connectivity-preprocessing)

3. [Building Functional Network](#building-functional-network)

4. [Graph Metrics Analysis](#graph-metrics=analysis)


## Setting up

Before we start, the following libraries are required for the functionalities of FnCA:
```{r,message=FALSE}
library("ANTsR")
library("ggplot2")
library("R.cache")  #storage for descriptive statistics
library("pracma")
library("mFilter")
library("igraph") 
library("dplyr")
...
We also start with the following data:
```{r}
...
It is also possible to summarize the entire network with a single, global measure. A global metric can be obtained by averaging all nodes. Global metrics include:
